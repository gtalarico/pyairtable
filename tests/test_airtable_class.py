...
@pytest.mark.parametrize("base_key,table_name,table_url_suffix", [
...
def test_get_all(table, mock_response_list, mock_records):
    with Mocker() as mock:
        mock.get(table.url_table, status_code=200, json=mock_response_list[0], complete_qs=True)
        for n, resp in enumerate(mock_response_list, 1):
            offset = resp.get('offset', None)
            if not offset:
                continue
            offset_url = table.url_table + '?offset={}'.format(offset)
            mock.get(offset_url, status_code=200, json=mock_response_list[1], complete_qs=True)
